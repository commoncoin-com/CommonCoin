...
#if defined(NDEBUG)
# error "CommonCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "CommonCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetCommonCoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // CommonCoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("commoncoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // CommonCoin: BIP30 has been active since inception
    ...
    // CommonCoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetCommonCoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // CommonCoin: Disallow AuxPow blocks before it is activated.
    ...
    // CommonCoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // CommonCoin: We probably want to disable this
    int nLockTimeFlags = 0;
