...
const char * const BITCOIN_CONF_FILENAME = "commoncoin.conf";
const char * const BITCOIN_PID_FILENAME = "commoncoind.pid";
...
#else
    const char* pszModule = "commoncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CommonCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CommonCoin";
#else
    // Unix
    return pathRet / ".commoncoin";
#endif
#endif
}
