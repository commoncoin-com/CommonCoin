...
// Copyright (c) 2022 The CommonCoin Core developers
...
UniValue getpeerinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "    \"feefilter\": \"decimal\",  (string)  The value of the last feefilter set by this peer, in COM\n"
...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addnode", "\"192.168.0.6:33555\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:33555\", \"onetry\"")
        );
...
UniValue disconnectnode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("disconnectnode", "\"192.168.0.6:33555\"")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:33555\"")
        );
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:33555\", (string) The commoncoin server IP and port we're connected to\n"
