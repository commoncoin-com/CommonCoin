...
const QString BITCOIN_IPC_PREFIX("commoncoin:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (QFile::exists(arg)) // Filename
        {
            ...
            {
                if (request.getDetails().genesis() == "00000e2c9f37bac91b7ed529282ac38c0efa9294504ebe77a5d4cff97869e265")
                {
                    SelectParams(CBaseChainParams::MAIN);
                }
                else if (request.getDetails().genesis() == "0000008e62079c1bce6d7ec265b4c1adff633d909fdce3fbc3a528f16205802a")
                {
                    SelectParams(CBaseChainParams::TESTNET);
                }
            }
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(ipcServerName)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start commoncoin: click-to-pay handler"));
        }
        else {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid CommonCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
