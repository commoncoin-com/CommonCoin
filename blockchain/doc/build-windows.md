...
Below are some notes on how to build CommonCoin Core for Windows.
...
To build CommonCoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the CommonCoin GitHub repository](https://github.com/commoncoin/commoncoin/)).
...
Note that for WSL v1 the CommonCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/commoncoin, AND not under, for example, /mnt/d/commoncoin.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for CommonCoin's dependencies):
...
way. This will install to `c:\workspace\commoncoin`, for example:

    make install DESTDIR=/mnt/c/workspace/commoncoin
