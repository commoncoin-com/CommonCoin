CommonCoin Core version 1.14.5 is now available from:

  <https://github.com/commoncoin/commoncoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All CommonCoin Core users, miners, services, relay
operators and wallet users are strongly recommended to upgrade.

Please report bugs using the issue tracker at github:

  <https://github.com/commoncoin/commoncoin/issues>

To receive security and update notifications, please watch reddit or Twitter:

  * https://www.reddit.com/r/commoncoindev/
  * @CommonCoin on Twitter for high priority announcements
  * @commoncoin\_devs on Twitter for updates on development work

Compatibility
==============

CommonCoin Core is extensively tested on Ubuntu Server LTS, Intel-based macOS
and Windows 10.

CommonCoin Core should also work on most other Unix-like systems but is not
...
most CommonCoin Core users.

### Remote Code Execution in CommonCoin QT (CVE-2021-3401)
...
**CommonCoin QT users are urged to please update their installations to this
...
leakage of sensitive information when CommonCoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**CommonCoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the CommonCoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 COM/kb, and
* the recommended dust limit is 1 COM, and
* the recommended RBF increment is 0.001 COM.

### Wallet/UI Changes

* The user interface for selecting fees when transacting COM has been updated
  to give an idea of how much is being spent, rather than a block target. As
  CommonCoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for CommonCoin.
...
* The default minimum transaction fee is now 0.01 COM per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 COM, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 COM.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 COM and sets the value under
...
The Berkley DB version used by CommonCoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within CommonCoin Core.
